...
(function ($) {
    $.fn.leftyrighty = function(options) {
        ...
        function popup() {
            if (settings.showPopup) { // show popup on document ready
        ...
        if(settings.addClassToFlipped) { // adding class for greater control when flipped
            $('.lr-toggle').on('click', function() {
                $(standardRow).toggleClass('lr-flipped-row');
                $(specificItem).toggleClass('lr-flipped-item');
            });
        }

        if (!settings.showToggleButton) { // show the toggle button
        ...
        if (settings.popupDelay != 0) { // popup delay
        ...
        if (settings.defaultHand == "left") { // default hand option (default is right)
